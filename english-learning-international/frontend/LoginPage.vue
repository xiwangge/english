...
// 动态加载微信 JS SDK 的辅助函数 (如果在 index.html 加了 script 标签，这个函数可以省略)
...
async function wechatLogin() {
  if (messageRef.value) {
    messageRef.value.innerHTML = ''; // 清空容器
    ...
    try {
      await loadWxLoginScript(); // 确保加载了微信 SDK
      const response = await fetch(`/api/wechat/qrcode?invitationCode=${invitationCode}`);
      const data = await response.json();

      if (data.appid && data.redirect_uri) {
        new window.WxLogin({
          self_redirect: false, // 必须为 false，否则是在 iframe 内跳转
          id: "message",
          appid: data.appid,
          scope: "snsapi_login",
          redirect_uri: data.redirect_uri, // 确保这个地址是经过 encodeURIComponent 的
          state: data.state,
          style: "", // 必须留空，用 href 覆盖
          href: customWechatStyle // <--- 注入自定义样式
        });
        
        // 【重要】这里不再调用 startPolling，完全依赖 postMessage
...
onUnmounted(() => { 
  // if (wechatLoginInterval) clearInterval(wechatLoginInterval); 
  window.removeEventListener('message', handleWechatMessage);
});
</script>
