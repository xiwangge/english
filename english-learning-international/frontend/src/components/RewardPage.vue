<template>
  <div class="reward-page-container" :class="{ 'dark': userStore.theme === 'dark' }">
...
onMounted(async () => {
  ...
  if (urlParams.get('status') === 'success') {
      toast.success('Payment successful! Welcome to membership.');
      await fetchUserInfo();
      // Clear URL params
...
/* 2. Dark Mode 样式覆盖 (改为 .dark 类) */
.reward-page-container.dark {
   /* 3. 强制深色背景 */
   background-color: var(--bg-color);
