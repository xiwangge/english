...
const songList = [
  {
    ...
    lightCues: [
      ...
      { time: 58.4, action: 'merged' },    
      { time: 107.2, action: 'scattered' },
    ],
  ...
  },
  {
    ...
    lightCues: [
      ...
      { time: 37.8, action: 'merged' },
      { time: 129.2, action: 'scattered' },
      { time: 141.7, action: 'merged' },  
    ],
...
/* --- 状态 B: 散开 (state-scattered) --- */
.spotlight-wrapper.state-scattered .spotlight-head { opacity: 1; }
.spotlight-wrapper.state-scattered .beam {
    opacity: 1;
    /* 应用基础散开角度，带有轻微的动态摇摆，增加活力 */
    animation: subtle-sway 6s infinite alternate ease-in-out;
}
/* 为每束光设置不同的摇摆延迟，避免同步 */
.spotlight-wrapper.state-scattered .beam-1 { animation-delay: 0s; }
.spotlight-wrapper.state-scattered .beam-2 { animation-delay: 1.5s; }
.spotlight-wrapper.state-scattered .beam-3 { animation-delay: 0.7s; }
.spotlight-wrapper.state-scattered .beam-4 { animation-delay: 2.2s; }


/* --- 状态 C: 合并 (state-merged) --- */
.spotlight-wrapper.state-merged .spotlight-head {
    opacity: 1;
    /* 合并时光源最强 */
    box-shadow: 
        0 0 20px 10px rgba(255, 255, 255, 1),
        0 0 40px 20px rgba(200, 230, 255, 0.9),
        0 0 80px 40px rgba(100, 200, 255, 0.6);
}
.spotlight-wrapper.state-merged .beam {
    opacity: 0.7; /* 叠加后亮度太高，稍微降低单体透明度 */
    filter: blur(18px); /* 加大模糊，让合并后的光柱更浑厚 */
    /* 强制聚拢到 0 度，并变宽 */
    transform: rotate(0deg) scaleX(3.5) !important; 
    /* 应用呼吸动画 */
    animation: breathe-merged 4s infinite alternate ease-in-out;
}
...
@keyframes breathe-merged {
    0% { transform: rotate(0deg) scaleX(3.5); opacity: 0.7; filter: blur(18px); }
    100% { transform: rotate(0deg) scaleX(4.2); opacity: 0.5; filter: blur(22px); }
}
...
.spotlight-head {
  ...
  box-shadow: 
    0 0 10px 4px #fff, 
    0 0 25px 8px rgba(150, 200, 255, 0.7), 
    0 0 50px 15px rgba(0, 100, 255, 0.3); 
  opacity: 0.9;
  transition: all 1s ease;
}
...
.beam {
  ...
  /* 【改动2】调整渐变，让中心核心区域更亮、更集中 */
  background: linear-gradient(
    to bottom,
    rgba(255, 255, 255, 0) 0%,    /* 根部极小透明 */
    rgba(255, 255, 255, 1) 2%,    /* 【核心】迅速达到最亮纯白，位置更靠前 */
    rgba(220, 245, 255, 0.7) 15%, /* 【核心】保持一段高亮区域 */
    rgba(150, 220, 255, 0.3) 50%, /* 中段亮度衰减 */
    rgba(100, 200, 255, 0.1) 80%, /* 远处变淡 */
    rgba(0, 0, 0, 0) 100%         /* 末尾消失 */
  );
  
  /* 稍微减小一点模糊，让细光束的边界稍微清晰一点点，更有力度 */
  filter: blur(12px);
...
/* --- 定义四个光束的基础角度 (保持不变) --- */
.beam-1 { --base-angle: -25deg; }
.beam-2 { --base-angle: -8deg; }
.beam-3 { --base-angle: 8deg; }
.beam-4 { --base-angle: 25deg; }
...
/* 应用旋转动画 (保持不变) */
.spotlight-wrapper.state-scattered .beam-1 { animation: rotate-beam-1 8s infinite alternate ease-in-out; }
.spotlight-wrapper.state-scattered .beam-2 { animation: rotate-beam-2 8s infinite alternate ease-in-out 0.5s; }
.spotlight-wrapper.state-scattered .beam-3 { animation: rotate-beam-3 8s infinite alternate ease-in-out 1s; }
.spotlight-wrapper.state-scattered .beam-4 { animation: rotate-beam-4 8s infinite alternate ease-in-out 1.5s; }

/* --- 状态 C: 合并变粗 (state-merged) --- */
.spotlight-wrapper.state-merged .spotlight-head {
    opacity: 1;
    box-shadow: 
        0 0 15px 5px #fff,
        0 0 30px 10px rgba(150, 200, 255, 0.8),
...
.spotlight-wrapper.state-merged .beam {
    opacity: 0.6; 
    filter: blur(20px); 
    /* 【改动3】合并时的变宽倍数稍微增加，因为基础变细了，需要更强的对比 */
    transform: rotate(0deg) scaleX(3.5); 
    animation: breathe-merged 4s infinite alternate ease-in-out;
}
...
@keyframes breathe-merged {
    /* 【改动4】配合上面的改动，调整呼吸动画的缩放比例 */
    0% { transform: rotate(0deg) scaleX(3.5); opacity: 0.6; }
    100% { transform: rotate(0deg) scaleX(4.0); opacity: 0.4; }
}
...
.spotlight-head {
  ...
  box-shadow: 
    ...
    0 0 30px 15px rgba(200, 230, 255, 0.8),
    0 0 60px 30px rgba(100, 200, 255, 0.5);
  opacity: 0.95;
  transition: all 1s ease;
}
...
/* 合并状态下，光束停止旋转，强制聚拢并变粗 */
.spotlight-wrapper.state-merged .beam {
    animation: none; /* 停止旋转动画 */
    opacity: 0.7;
    filter: blur(15px);
    transform: rotate(0deg) scaleX(3) !important; /* 强制聚拢并变宽 */
    animation: breathe-merged 4s infinite alternate ease-in-out; /* 应用呼吸动画 */
}
